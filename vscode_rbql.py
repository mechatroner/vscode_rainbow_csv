...
def run_with_python(args):
    ...
    rbql_lines = None
    if query is None:
        print_error_and_exit('Error: provide "--query" option')
    rbql_lines = [query]

    with rbql.RbqlPyEnv() as worker_env:
        tmp_path = worker_env.module_path
        try:
            rbql.parse_to_py(rbql_lines, tmp_path, delim, policy, output_delim, output_policy, csv_encoding, None)
...
def run_with_js(args):
    ...
    rbql.parse_to_js(input_path, output_path, rbql_lines, tmp_path, delim, policy, output_delim, output_policy, csv_encoding, None)
    cmd = ['node', tmp_path]
